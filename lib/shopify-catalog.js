...
function shopDescription(product) {
  const description = textFromHtml(product.descriptionHtml);
  if (product.handle !== 'chopped-craw-6-pack') return description;
  return description.replace(/\bJewell? Baits\b/gi, 'Jewel Bait Company');
}
...
function normalizeDrop(product, rattle, errors, now) {
  ...
  return {
    ...
    shopifyCategory: product.shopifyCategory || product.category || null,
    ...
    rattle: rattle && taxonomy.categoryForProduct(product) === 'jigs' ? {
...
function normalizeDiscoveredProduct(product, rattle) {
  ...
  return {
    ...
    productType: product.productType || '',
    tags: Array.isArray(product.tags) ? product.tags.slice() : [],
    shopifyCategory: product.shopifyCategory || null,
    category: taxonomy.categoryForProduct(product),
    search: [
      product.title,
      product.vendor,
    ...
    } : { available: false, defaultKey: 'no', options: [
      { key: 'no', label: 'No', priceDelta: 0 }
    ] },
    weights,
    colors,
...
async function loadFreshCatalog(request, options = {}) {
  ...
  const collarProduct = admittedRemoteByHandle.get('wire-tied-skirt-collar-add-on');
  const collar = normalizeRattle(collarProduct);
  if (collar && (collar.price !== 2 || collar.currencyCode !== 'USD')) collar.available = false;
  ...
  const products = envelope.products
    ...
    .map((product) => normalizeDiscoveredProduct(product, rattle))
    .filter(Boolean);
