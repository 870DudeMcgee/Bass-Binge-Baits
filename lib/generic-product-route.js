...
const productEditorial = require('./product-editorial.js');
const {
  merchantReturnPolicy,
  shippingDetailsForOffer,
  shippingRate,
} = require('./product-policies.js');
...
function imageForVariant(product, variant) {
  if (variant && variant.image && variant.image.url) {
    return {
      type: 'image',
      alt: variant.image.alt || product.title,
      image: variant.image
    };
  }
  const imageId = variant && variant.imageId;
  return (Array.isArray(product && product.media) ? product.media : []).find((item) =>
    imageId &&
    item &&
    item.type === 'image' &&
    item.image &&
    item.image.url &&
    (item.id === imageId || item.image.id === imageId)
  ) || firstImage(product);
}
...
function defaultVariant(product) {
  ...
  return variants.find((variant) => variant && variant.availableForSale) || variants[0] || null;
}

function selectVariant(product, requestedId) {
  const fallback = defaultVariant(product);
  const rawRequested = Array.isArray(requestedId) ? requestedId[0] : requestedId;
  const requested = rawRequested == null ? '' : String(rawRequested);
  if (!/^\d+$/.test(requested)) {
    return { variant: fallback, requested: Boolean(requested), valid: !requested };
  }
  const variant = (product.variants || []).find((candidate) => variantNumericId(candidate) === requested);
  return { variant: variant || fallback, requested: true, valid: Boolean(variant) };
}
...
function offerForVariant(product, variant, canonical) {
  if (!variant || !variant.price || variant.price.amount == null || !variant.price.currencyCode) return null;
  const numericId = variantNumericId(variant);
  const rate = shippingRate(product, variant);
  return {
    '@type': 'Offer',
    url: numericId ? `${canonical}?variant=${numericId}` : canonical,
    ...
    ...(rate ? { shippingDetails: shippingDetailsForOffer(rate, variant) } : {}),
    hasMerchantReturnPolicy: merchantReturnPolicy(),
  };
}

function productGroupId(product) {
  return String(product && (product.id || product.handle) || '');
}

function schemaOptionProperty(product, name) {
  const normalized = String(name || '').trim().toLowerCase();
  // These live Shopify option labels contain colors, despite their legacy names.
  if (product.handle === '3-4-oz-football-jig' && normalized === '3/4 oz.') return 'color';
  if (product.handle === 'pee-wee-football' && normalized === 'pee wee + colors') return 'color';
  if (normalized === 'color' || normalized === 'colour') return 'color';
  if (normalized === 'size' || normalized === 'weight') return 'size';
  if (normalized === 'material' || normalized === 'pattern') return normalized;
  return null;
}

function schemaVariantProperties(product, variant) {
  return (variant && Array.isArray(variant.selectedOptions) ? variant.selectedOptions : [])
    .reduce((properties, option) => {
      ...
      const property = schemaOptionProperty(product, name);
      if (property) properties[property] = value;
      return properties;
    }, {});
}

function schemaVariesBy(product) {
  const properties = (product && Array.isArray(product.options) ? product.options : [])
    .map((option) => String(option && option.name || '').trim().toLowerCase())
    .map((name) => schemaOptionProperty(product, name))
    .filter(Boolean);
  return [...new Set(properties)].map((property) => `https://schema.org/${property}`);
}

function productStructuredData(product, description, image, canonical) {
  ...
  if (image && image.image && image.image.url) common.image = image.image.url;
  if (product.vendor) common.brand = { '@type': 'Brand', name: product.vendor };
  ...
  if (variants.length > 1) {
    ...
    return {
      ...
      variesBy: schemaVariesBy(product),
      hasVariant: variants.map((variant) => {
        const offer = offerForVariant(product, variant, canonical);
        const selected = (variant.selectedOptions || []).map((option) => option.value).filter(Boolean);
        const variantImage = imageForVariant(product, variant);
        const item = {
          '@type': 'Product',
          '@id': `${offer ? offer.url : canonical}#product`,
          name: selected.length ? `${product.title} - ${selected.join(' / ')}` : product.title,
          description: description || product.title,
          inProductGroupWithID: groupId,
          ...schemaVariantProperties(product, variant)
        };
        if (variantImage && variantImage.image && variantImage.image.url) {
          item.image = variantImage.image.url;
        }
        if (variant.sku) item.sku = String(variant.sku);
  ...
  const offer = offerForVariant(product, variants[0], canonical);
  return {
    ...common,
    '@type': 'Product',
    '@id': `${canonical}#product`,
    ...(variants[0] && variants[0].sku ? { sku: String(variants[0].sku) } : {}),
    ...(offer ? { offers: offer } : {})
  };
}
...
function renderGenericProductPage(product, requestedVariantId) {
  const displayProduct = {
    ...product,
    title: correctJewelSpelling(product.title),
    descriptionHtml: correctJewelSpelling(product.descriptionHtml)
  };
  const editorial = productEditorial[displayProduct.handle] || {};
  const variantSelection = selectVariant(displayProduct, requestedVariantId);
  const selectedVariant = variantSelection.variant;
  const description = textFromHtml(displayProduct.descriptionHtml);
  const image = imageForVariant(displayProduct, selectedVariant);
  const productJson = jsonScript(displayProduct);
  const canonical = `https://www.bassbingebaits.com/products/${encodeURIComponent(displayProduct.handle)}`;
  const structuredData = jsonScript(productStructuredData(displayProduct, description, image, canonical));
  const visiblePrice = selectedVariant ? formatPrice(selectedVariant.price) : '';
  const availabilityText = selectedVariant && selectedVariant.availableForSale
...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="/assets/css/product.css?v=20260906-video-gallery" />
  </head>
  <body data-generic-product>
      ...
      <div class="container">
        <section class="product-hero">
          <div class="product-hero-left">
            <div class="product-gallery" data-gallery aria-label="${escapeHtml(displayProduct.title)} image gallery">
              <div class="product-gallery-main">
                <div class="product-gallery-track" data-gallery-track>${image ? `<div class="product-gallery-slide active"><img src="${escapeHtml(image.image.url)}" alt="${escapeHtml(image.alt || displayProduct.title)}" /></div>` : ''}</div>
          ...
          <div class="product-hero-right">
            ...
            <p class="product-hero-price" data-price-display>${escapeHtml(visiblePrice)}</p>
            <p class="product-availability${selectedVariant && !selectedVariant.availableForSale ? ' is-unavailable' : ''}" data-product-availability aria-live="polite">${escapeHtml(availabilityText)}</p>
            ${description ? `<p class="product-hero-desc">${escapeHtml(description)}</p>` : ''}
            <div data-generic-options></div>
            <noscript><p>Enable JavaScript to choose product options and add this item to your cart.</p></noscript>
            ${editorial.specs || ''}
            <div class="product-purchase">
              ...
              <button class="btn btn-primary" type="button" data-add-cart${selectedVariant && selectedVariant.availableForSale ? '' : ' disabled'}>${selectedVariant && selectedVariant.availableForSale ? 'Add to Cart' : 'Unavailable'}</button>
              <a href="/shop" class="btn btn-secondary">View All Products</a>
            </div>
            ${editorial.brand || jewelCrawBranding}
          </div>
        </section>
        ${editorial.details || ''}
      </div>
    ...
    <aside class="cart-drawer" data-cart-drawer aria-hidden="true" aria-labelledby="cart-title">
      ...
      <div class="cart-drawer-footer">
        ...
        <p class="cart-note">Shipping and taxes are calculated during checkout. <a href="/returns">Returns</a></p>
    ...
    <footer class="site-footer">
      <div class="container footer-row">
        ...
        <div class="footer-links"><a href="/contact">Contact</a><a href="/about">About</a><a href="/shop">Shop</a><a href="/returns">Returns</a><a href="/privacy">Privacy</a></div>
      </div>
    </footer>

    <script id="generic-product-jsonld" type="application/ld+json">${structuredData}</script>
    <script id="generic-product-data" type="application/json">${productJson}</script>
    <script src="/assets/js/catalog-taxonomy.js?v=20260821-live-merch-routing" defer></script>
    <script src="/assets/js/catalog.js?v=20260911-skirt-collar" defer></script>
    <script src="/assets/js/cart-checkout.js?v=20260911-skirt-collar" defer></script>
    <script src="/assets/js/main.js?v=20260722" defer></script>
    <script src="/assets/js/generic-product-page.js?v=20260911-skirt-collar" defer></script>
  </body>
</html>`;
}
...
function createGenericProductHandler(dependencies) {
  ...
  return async function genericProductHandler(request, response) {
    ...
    try {
      ...
      const rawVariant = request.query && request.query.variant;
      return response.status(200).send(renderGenericProductPage(product, rawVariant));
    } catch (error) {
