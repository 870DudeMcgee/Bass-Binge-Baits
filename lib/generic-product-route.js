...
function renderGenericProductPage(product) {
  ...
  const canonical = `https://www.bassbingebaits.com/products/${encodeURIComponent(product.handle)}`;
  const productJson = serializeJson(product);
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(product.title)} | Bass Binge Baits</title>
    <meta name="description" content="${escapeHtml(description.slice(0, 160) || product.title)}" />
    <meta property="og:title" content="${escapeHtml(product.title)} | Bass Binge Baits" />
    <meta property="og:description" content="${escapeHtml(description.slice(0, 200) || product.title)}" />
    ...
    <link rel="stylesheet" href="/assets/css/product.css?v=20260727-generic-zoom" />
  </head>
  <body data-generic-product>
      ...
      <div class="container">
        <section class="product-hero">
          <div class="product-hero-left">
            <div class="product-gallery" data-generic-gallery aria-label="${escapeHtml(product.title)} image gallery">
              <div class="product-gallery-main" data-gallery-main>
                <div class="product-gallery-track" data-gallery-track></div>
                <button class="product-gallery-zoom-toggle" type="button" data-gallery-zoom-open aria-label="Open larger product photo" hidden>
                ...
                <button class="product-gallery-arrow prev" type="button" data-gallery-prev aria-label="Previous media">
                  <svg aria-hidden="true" viewBox="0 0 24 24"><path d="m15 18-6-6 6-6" /></svg>
                </button>
                <button class="product-gallery-arrow next" type="button" data-gallery-next aria-label="Next media">
          ...
          <div class="product-hero-right">
            <p class="section-kicker">${escapeHtml(product.productType || product.vendor || 'Bass Binge Baits')}</p>
            <h1 class="product-hero-title">${escapeHtml(product.title)}</h1>
    ...
    <div class="product-zoom-modal" data-product-zoom-modal role="dialog" aria-modal="true" aria-label="${escapeHtml(product.title)} enlarged product photo" hidden>
      <div class="product-zoom-backdrop" data-product-zoom-close></div>
      <div class="product-zoom-panel">
        <button class="product-zoom-close" type="button" data-product-zoom-close aria-label="Close product photo zoom">
          <svg aria-hidden="true" viewBox="0 0 24 24"><path d="m6 6 12 12M18 6 6 18" /></svg>
        </button>
        <button class="product-zoom-arrow prev" type="button" data-product-zoom-prev aria-label="Previous image">
          <svg aria-hidden="true" viewBox="0 0 24 24"><path d="m15 18-6-6 6-6" /></svg>
        </button>
        <div class="product-zoom-stage" data-product-zoom-stage tabindex="0" aria-label="Tap or click to zoom the product photo">
          <img class="product-zoom-img" data-product-zoom-image src="" alt="" draggable="false" />
        </div>
        <button class="product-zoom-arrow next" type="button" data-product-zoom-next aria-label="Next image">
          <svg aria-hidden="true" viewBox="0 0 24 24"><path d="m9 6 6 6-6 6" /></svg>
        </button>
        <div class="product-zoom-meta">
          <span class="product-zoom-caption" data-product-zoom-caption></span>
          <span class="product-zoom-counter" data-product-zoom-counter></span>
        </div>
      </div>
    </div>
    ...
    <script src="/assets/js/catalog.js?v=20260726-envelope-v2" defer></script>
    <script src="/assets/js/cart-checkout.js?v=20260726-generic" defer></script>
    <script src="/assets/js/main.js?v=20260722" defer></script>
    <script src="/assets/js/generic-product-page.js?v=20260727-generic-zoom" defer></script>
  </body>
</html>`;
}
