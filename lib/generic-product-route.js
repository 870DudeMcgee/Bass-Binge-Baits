...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="/assets/css/product.css?v=20260726-generic" />
  </head>
  <body data-generic-product>
    ...
    <script src="/assets/js/generic-product-page.js?v=20260726-generic" defer></script>
  </body>
</html>`;
}
...
function createGenericProductHandler(dependencies) {
  ...
  return async function genericProductHandler(request, response) {
    ...
    try {
      ...
      response.setHeader('Cache-Control', 'no-store');
      response.setHeader('Content-Type', 'text/html; charset=utf-8');
      return response.status(502).send(renderStatusPage(502, 'Product temporarily unavailable', 'Please try again in a moment.'));
    }
  };
}
