...
function createCatalogService(options = {}) {
  ...
  if (!store || typeof store.get !== 'function' || typeof store.set !== 'function') {
    throw new TypeError('createCatalogService requires a durable store');
  }
  ...
  async function refresh(request, observedAt, refreshOptions = {}) {
    ...
    try {
      ...
      if (typeof store.commitRefresh === 'function') {
        await store.commitRefresh({
          recordKey,
          record,
          dirtyKey,
        ...
        if (refreshOptions.dirtyRaw && typeof store.deleteIfValue === 'function') {
          await store.deleteIfValue(dirtyKey, refreshOptions.dirtyRaw);
        }
      }
      return decorate(record, observedAt, refreshOptions.cache || 'miss', 'fresh');
    } finally {
  ...
  return {
    async getCatalog(request) {
      ...
      try {
        return await refresh(request, observedAt, { dirtyRaw });
      } catch (error) {
    ...
    async runScheduledRefresh(request, scheduleToken) {
      const dirtyRaw = await store.get(dirtyKey);
      if (!dirtyRaw) {
        if (scheduleToken && typeof store.deleteIfValue === 'function') {
          await store.deleteIfValue(refreshScheduleKey, scheduleToken);
        }
        return null;
      }
      try {
        ...
        if (scheduleToken && typeof store.deleteIfValue === 'function') {
          try {
            ...
            logger.error('Catalog refresh schedule release failed', { message: error.message });
          }
        }
      }
    },
