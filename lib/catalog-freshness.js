  ...
  return {
    async getCatalog(request) {
      ...
      try {
        ...
        const failedAt = now();
        if (record && ageMs(record, failedAt) < staleMs) {
          return decorate(record, failedAt, 'stale', 'stale', {
            dirty: Boolean(dirty),
            dirtyAt: dirty && dirty.markedAt,
            refreshDueAt: dirty && dirty.dueAt
          });
        }
        if (error instanceof CatalogUnavailableError && error.details.reason === 'refresh_in_progress') {
          // ponytail: follow the durable owner for at most eight seconds; never steal its lease.
          const deadline = now() + 8000;
          let timeout;
          try {
            return await Promise.race([
              new Promise((resolve, reject) => { timeout = setTimeout(() => reject(error), 8000); }),
              (async () => {
                for (let attempt = 0; attempt < 32 && now() < deadline; attempt += 1) {
                  await delay(Math.min(250, deadline - now()));
                  const [completed, currentDirtyRaw] = await Promise.all([
                    store.get(recordKey), store.get(dirtyKey)
                  ]);
                  const currentDirty = currentDirtyRaw ? JSON.parse(currentDirtyRaw) : null;
                  const checkedAt = now();
                  if (completed && ageMs(completed, checkedAt) < staleMs) {
                    const fresh = ageMs(completed, checkedAt) < ttlMs &&
                      (!currentDirty || Date.parse(currentDirty.dueAt) > checkedAt);
                    return decorate(completed, checkedAt, fresh ? 'hit' : 'stale', fresh ? 'fresh' : 'stale', {
                      dirty: Boolean(currentDirty),
                      dirtyAt: currentDirty && currentDirty.markedAt,
                      refreshDueAt: currentDirty && currentDirty.dueAt
                    });
                  }
                }
                throw error;
