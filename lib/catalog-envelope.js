...
function normalizeShopifyCategory(category) {
  ...
  return {
    ...
    ancestors: (Array.isArray(category.ancestors) ? category.ancestors : [])
      .filter((ancestor) => ancestor && ancestor.name)
      .map((ancestor) => ({
        id: ancestor.id || null,
...
function classifyProduct(product) {
  ...
  if (['rattle-add-on', 'wire-tied-skirt-collar-add-on'].some((type) => productType === type || tags.includes(type))) return 'hidden-add-on';
...
function presentationFor(product) {
  ...
  const productCategory = taxonomy.categoryForProduct(product);
  ...
  return {
    ...
    rattleEnabled: kind !== 'hidden-add-on' &&
      (productCategory === 'jigs' ||
        (Array.isArray(product.tags) ? product.tags : []).map(normalizeKey).includes('rattle-enabled'))
  };
}
...
function normalizeProduct(product, generatedAt) {
  ...
  const normalized = {
    ...
    shopifyCategory: normalizeShopifyCategory(product.category),
