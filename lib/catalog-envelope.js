...
function presentationFor(product) {
  ...
  return {
    ...
    dropStartsAt: fields.drop_starts_at || null,
...
function normalizeProduct(product, generatedAt) {
  ...
  const media = product && product.media && Array.isArray(product.media.nodes)
    ...
    : [];
  if (!media.some((item) => item.type === 'image' && item.image && item.image.url)) {
    quarantine.push(issueFor(product, generatedAt, {
      severity: 'warning',
  ...
  const normalized = {
    ...
    featuredMediaId: product.featuredMedia && product.featuredMedia.id || null,
    media,
    options,
    variants,
    presentation
