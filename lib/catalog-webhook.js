...
async function readRawBody(request) {
  if (Buffer.isBuffer(request && request.rawBody)) return request.rawBody;
  if (typeof (request && request.rawBody) === 'string') return Buffer.from(request.rawBody);
...
function createCatalogWebhookHandler(options = {}) {
  ...
  return async function catalogWebhookHandler(request, response) {
    ...
    if (!signaturesMatch(rawBody, header(request, 'x-shopify-hmac-sha256'), secret)) {
